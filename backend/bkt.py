...
class BKTDoctor:
    ...
    def update_mastery(self, current_mastery: float, effective_correctness: float) -> float:
        """
        ...
        p_l_given_correct = (current_mastery * (1 - self.p_slip)) / \
                            (current_mastery * (1 - self.p_slip) + (1 - current_mastery) * self.p_guess)
                            
        # Standard update if fully incorrect (effective == 0.0)
        p_l_given_incorrect = (current_mastery * self.p_slip) / \
                              (current_mastery * self.p_slip + (1 - current_mastery) * (1 - self.p_guess))
        ...
        new_mastery = p_l_given_evidence + (1 - p_l_given_evidence) * self.p_learn
